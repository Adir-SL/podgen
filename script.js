function genXML() {
    ...
    var monthNumber = date.slice(date.indexOf("-") + 1, date.lastIndexOf("-"));
    var monthArray = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
    var month = monthArray[Number(monthNumber) - 1];
    var time = document.getElementById("time").value + ":00 GMT";
    var date = "Sun, " + day + " " + month + " " + year;
    ...
    var i;
    for (i = 0; i < number; i++) {
        document.getElementById("xml").innerText += '\n<item>\n<author>The Feed</author>\n<itunes:author>The Feed</itunes:author>\n<title>' + name + ' Part ' + (i + 1) + '</title>\n <pubDate>' + date + ' ' + time + '</pubDate>\n <enclosure url = "' + domain + '/' + filename + '_' + (i + 1) + type + '" type = "audio/mpeg" />\n <itunes:duration>' + duration + '</itunes:duration>\n <guid isPermaLink = "false">cepod' + (i + 1) + '</guid>\n <itunes:explicit>no</itunes:explicit>\n <description>\nThe ever changing feed.\n</description>\n</item>\n';
    }
